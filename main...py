...
def main():
    ...
    <h2 style = "color:black;text-align:center;"> Drex Dang Car Price Prediction Web App </h2>
    </div>
    """
    
    model = xgb.XGBRegressor()
    model.load_model('C:/Users/Drex/Desktop/Car Price Web App/xg_model.json')
    ...
    try:
        if st.button ('Predict'):
            ...
            if pred > 0:
                st.balloons()
                st.success('You can sell your car for $ {:.2f} '.format (pred[0]))
            else:
